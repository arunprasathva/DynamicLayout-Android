...
public class LinePageIndicator extends View implements PageIndicator {
    ...
    private static final int LINE_INDICATOR_MAX = 4;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int actualCount = mViewPager.getAdapter().getCount();
        int count = actualCount;
        if (count == 0) {
            return;
        } else if (count > LINE_INDICATOR_MAX) {
            count = LINE_INDICATOR_MAX;
        }

        if (mCurrentPage >= actualCount) {
        ...
        int optiPageNo = mCurrentPage % LINE_INDICATOR_MAX;
        //Draw stroked circles
        for (int i = 0; i < count; i++) {
            float dx1 = horizontalOffset + (i * lineWidthAndGap);
            float dx2 = dx1 + mLineWidth;
            canvas.drawLine(dx1, verticalOffset, dx2, verticalOffset, (i == optiPageNo) ? mPaintSelected : mPaintUnselected);
        }
    }
